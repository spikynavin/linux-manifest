...
This is Linux manifest for raspberry pi b3+ Yocto development.
...
***Note: These host dependence packages are for kirkstone version and also test on ubuntu-18.04 & 20.04 LTS distro.***
...
#### Sync source
```bash
repo init -u https://github.com/spikynavin/linux-manifest -b main -m platform-mainline.xml --repo-url=https://github.com/GerritCodeReview/git-repo  --repo-branch=stable --no-repo-verify
```
```bash
repo sync -c --no-tags
```
...
```bash
ls workspace/build-sdk/tmp-glibc/deploy/image/
ls workspace/build-sdk/tmp-glibc/deploy/sdk/
```
